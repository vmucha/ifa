...
module.exports = {
    ...
    getContent: function (req, res) {
        ...
        Artikel.findOne({id: req.param('id')},function(err,found){
            if (found.type !== 'xhtml') {
                request('http://www.welt.de/article' + found.id + '/?noredirect=true&config=jsn&' + Math.random(), function (error, response, body) {
                ...
                request('http://www.welt.de/article' + found.id + '/?noredirect=true&config=standalone&' + Math.random(), function (error, response, body) {
                    console.log(body);
                    Artikel.update({id: found.id},{
                        body: body
                    }, function(err, updated){
                        res.send(updated[0]);
                    });
